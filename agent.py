...
def select_max(board, color, limit, caching):
    if caching:
        if (hash(board), color, "max") in cache:
            cached_limit, cached_move, cached_move_util = cache[(hash(board), color, "max")]
            # check if the cached value is obtained from depth limit or greater and use it if it is.
            if cached_limit >= limit:
                return cached_move, cached_move_util
    ...
    if caching:
        if (hash(board), color, "max") in cache:
            # if some value already exist, then check for its limit and overwrite cache if we found a move from a deeper depth.
            cached_limit, cached_move, cached_move_util = cache[(hash(board), color, "max")]
            if limit >= cached_limit:
                cache[(hash(board), color, "max")] = limit, max_move, max_util
        else:
            # not cached yet, just cache it.
            cache[(hash(board), color, "max")] = limit, max_move, max_util
    return max_move, max_util
    
def select_min(board, color, limit, caching):
    if caching:
        if (hash(board), color, "min") in cache:
            cached_limit, cached_move, cached_move_util = cache[(hash(board), color, "min")]
            # check if the cached value is obtained from depth limit or greater and use it if it is.
            if cached_limit >= limit:
                return cached_move, cached_move_util
    moves = get_possible_moves(board, color)
    ...
    if caching:
        if (hash(board), color, "min") in cache:
            ...
            cached_limit, cached_move, cached_move_util = cache[(hash(board), color, "min")]
            if limit >= cached_limit:
                cache[(hash(board), color, "min")] = limit, min_move, min_util
        else:
            # not cached yet, just cache it.
            cache[(hash(board), color, "min")] = limit, min_move, min_util
